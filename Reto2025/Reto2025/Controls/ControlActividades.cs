...
namespace Reto2025.Controls
{
    internal class ControlActividades
    {
        ...
        public async Task<bool> GuardarActividad(Actividad actividad)
        {
            try
            {
                // Convertir el objeto Realizada a JSON
                ...
                var content = new StringContent(json, Encoding.UTF8, "application/json");
                // Realizar la solicitud POST a la API
                HttpResponseMessage response = await client.PostAsync($"http://localhost:8080/acex/actividades", content);
