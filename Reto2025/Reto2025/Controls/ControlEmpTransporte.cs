...
namespace Reto2025.Controls
{
    internal class ControlEmpTransporte
    {
        ...
        public async Task<bool> GuardarEmpresa(EmpTransporte empTransporte)
        {
            try
            {
                // Convertir el objeto Realizada a JSON
                ...
                var content = new StringContent(json, Encoding.UTF8, "application/json");
                // Realizar la solicitud POST a la API
                HttpResponseMessage response = await client.PostAsync($"http://localhost:8080/acex/transportes", content);
